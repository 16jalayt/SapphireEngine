...
void Player::open()
{
	...
	// check video
	if (res != 0)
		FFUtils::display_ffmpeg_exception(res);
	...
	if (videoStream == -1)
		FFUtils::display_exception("Error opening your video using AVCodecParameters, probably doesnt have codecpar_type type AVMEDIA_TYPE_VIDEO");
...
int Player::get_video_stream(void)
{
	...
	if (videoStream == -1)
		FFUtils::display_exception("Couldnt find stream");
...
int Player::read_audio_video_codec(void)
{
	...
	if (pCodec == NULL)
		FFUtils::display_exception("Video decoder not found");

	if (pAudioCodec == NULL)
		FFUtils::display_exception("Audio decoder not found");

	pCodecCtx = avcodec_alloc_context3(pCodec);

	if (pCodecCtx == NULL)
		FFUtils::display_exception("Failed to allocate video context decoder");

	pCodecAudioCtx = avcodec_alloc_context3(pAudioCodec);

	if (pCodecAudioCtx == NULL)
		FFUtils::display_exception("Failed to allocate audio context decoder");

	int res = avcodec_parameters_to_context(pCodecCtx, pCodecParameters);

	if (res < 0)
		FFUtils::display_exception("Failed to transfer video parameters to context");

	res = avcodec_parameters_to_context(pCodecAudioCtx, pCodecAudioParameters);

	if (res < 0)
		FFUtils::display_exception("Failed to transfer audio parameters to context");

	res = avcodec_open2(pCodecCtx, pCodec, NULL);

	if (res < 0)
		FFUtils::display_exception("Failed to open video codec");

	res = avcodec_open2(pCodecAudioCtx, pAudioCodec, NULL);

	if (res < 0)
		FFUtils::display_exception("Failed to open auvio codec");
...
int Player::malloc(void)
{
	...
	FFAudio::get_instance()->open();

	pFrame = av_frame_alloc();
	if (pFrame == NULL)
		FFUtils::display_exception("Couldnt allocate frame memory");

	pFrameRGB = av_frame_alloc();
	if (pFrameRGB == NULL)
		FFUtils::display_exception("Couldnt allocate rgb frame memory");
...
int Player::display_video(void) {
	...
	while (av_read_frame(pFormatCtx, &packet) >= 0) {
		if (packet.stream_index == audioStream) {
			FFAudio::get_instance()->put_audio_packet(&packet);
		}
		...
		if (packet.stream_index == videoStream)
		{
			...
			SDL_UpdateWindowSurface(Graphics::window.get());
			SDL_Delay(1000 / 30);
		}
