...
FFAudio* FFAudio::instance = 0;

FFAudio* FFAudio::get_instance()
{
	if (instance == 0)
		instance = new FFAudio();
	return instance;
}
...
void FFAudio::open()
{
	...
	if (SDL_OpenAudio(&wantedSpec, &audioSpec) < 0)
		FFUtils::display_exception("Failed to open audio");
...
void FFAudio::init_audio_packet(AudioPacket* q)
{
	q->last = NULL;
	q->first = NULL;
...
void FFAudio::malloc(AVCodecContext* pCodecAudioCtx)
{
	...
	if (res != 0)
		FFUtils::display_exception("Failed to initialize audio");
...
int FFAudio::audio_decode_frame(AVCodecContext* aCodecCtx, uint8_t* audio_buf, int buf_size) {
	...
	while (1)
	{
		while (audio_pkt_size > 0)
		{
			...
			if (len2 < 0)
				FFUtils::display_exception("swr_convert failed");
